    ...
    <Card className="hover:shadow-lg transition-shadow duration-200">
      <CardContent className="p-4 lg:p-6">
        <div className="flex items-center gap-2 mb-4 lg:mb-6">
          ...
          <h3 className="font-semibold text-card-foreground text-base lg:text-lg">Conoce a tus Candidatos</h3>
        </div>
        
        <div className="space-y-3 lg:space-y-4">
          {candidates.map((candidate) => (
            <div key={candidate.id} className="flex items-center gap-3 lg:gap-4 p-3 lg:p-4 bg-muted/50 rounded-lg hover:bg-muted transition-colors">
              ...
              <div className="flex-1">
                <p className="font-medium text-card-foreground text-sm lg:text-base">{candidate.name}</p>
                <p className="text-sm lg:text-base text-muted-foreground">{candidate.party}</p>
              </div>
            </div>
          ))}
        </div>
